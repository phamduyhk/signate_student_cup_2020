...
def build_model(max_seq_length):
    ...
    gru_out = tf.keras.layers.GRU(100, recurrent_dropout=0.2, return_sequences=True)(bert_output)
    ...
    dense = tf.keras.layers.Dense(320, activation='relu')(conc)
...
def main():
    ...
    train_label = train_df["jobflag"].values

    onehot_encoder = OneHotEncoder(sparse=False)
    reshaped = train_label.reshape(len(train_label), 1)
    y_train_onehot = onehot_encoder.fit_transform(reshaped)

    X_tra, X_val, y_tra, y_val = train_test_split(train_text, y_train_onehot, train_size=0.95, random_state=233)

    test_text = test_df["description"].tolist()
    test_text = [" ".join(t.split()[0:max_seq_length]) for t in test_text]
    test_text = np.array(test_text, dtype=object)[:, np.newaxis]

    y_test_onehot = y_train_onehot[:len(test_text)]
    ...
    test_examples = convert_text_to_examples(test_text, y_test_onehot)
